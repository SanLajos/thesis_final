...
from chatbot_engine import chat_with_tutor # <--- CHATBOT IMPORT
...
# FIX: Robust CORS - Allow all origins to stop Network Errors during dev
...
def register():
    ...
    try:
        ...
        # Return tokens in JSON body (Browser independent)
        return jsonify({
...
def login():
    try:
        data = request.json
        validated = UserLoginSchema(**data) # Uses 'identifier' now
    except ValidationError as e: return jsonify({"error": str(e)}), 400
...
def refresh_token():
    # Expect refresh token in JSON body
...
@limiter.limit("10 per minute") # Strict rate limit
def chat():
    ...
    history = data.get('history', []) # Expect frontend to send conversation context
...
def create_assignment():
    ...
    try:
        ...
        # Handle test_cases
        if 'test_cases' not in form_data or not form_data['test_cases']:
...
def submit_for_grading(assignment_id):
    ...
    try:
        ...
        if assignment.get('grading_type') == 'keyword':
            ...
            exec_context = ""
            if test_results:
                exec_context = f"\n[Execution Results]: Passed {test_results['passed']}/{test_results['total']} tests."
            
            grade_result = grade_with_ai(generated_code, template_code, assignment['description'], assignment.get('grading_prompt', '') + exec_context)
