...
function StructogramApp() {
    ...
    const checkSession = async () => {
        if (api.token) {
            ...
            if (res.ok) { 
                ...
                // If on root or login, go to seminars
            ...
            // If strictly public route, allow it, else redirect
            if (!['/login', '/register'].includes(location.pathname)) {
               navigate('/login');
            }
        }
  ...
  // Fetch assignments logic when navigating
  useEffect(() => {
  ...
  // This allows child components (like SeminarList) to keep using "setView('dashboard')" 
  // without needing to be rewritten immediately.
    ...
    <div className="min-h-screen bg-[#F5F7FA] pb-20">
      ...
      <Routes>
        ...
        <Route path="/dashboard" element={
            // Safety Check: If refreshed on dashboard without state, go back to list
            user && activeSeminar ? (
                <AssignmentDashboard 
                  activeSeminar={activeSeminar} setView={compatibilitySetView} user={user} 
        ...
        <Route path="/create" element={
            user ? <AssignmentCreate activeSeminar={activeSeminar} setView={compatibilitySetView} refreshAssignments={refreshAssignments} /> : <Navigate to="/login" />
        } />
        
        <Route path="/submissions" element={
            user ? <SubmissionList submissionsList={submissionsList} setView={compatibilitySetView} setGradingResult={setGradingResult} currentAssignment={currentAssignment} /> : <Navigate to="/login" />
        } />
        
        <Route path="/submit" element={
            user ? <StudentSubmission currentAssignment={currentAssignment} setView={compatibilitySetView} setGradingResult={setGradingResult} /> : <Navigate to="/login" />
        } />
        
        <Route path="/grading" element={
            user ? <GradingResult gradingResult={gradingResult} setView={compatibilitySetView} /> : <Navigate to="/login" />
        } />

        {/* Fallback */}
        <Route path="*" element={<Navigate to={user ? "/seminars" : "/login"} />} />
      </Routes>
