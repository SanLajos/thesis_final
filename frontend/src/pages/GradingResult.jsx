...
export const GradingResult = ({ gradingResult, setView }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pt-6">
      <button onClick={() => setView('dashboard')} className="text-slate-500 hover:text-[#1B3147] transition-colors">&larr; Back to Dashboard</button>
      
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-[#1B3147]">Grading Report</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
             ...
             <div className={`p-4 rounded border ${grading_result.is_correct ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
               ...
               <p className="mt-2 text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{grading_result.feedback}</p>
             </div>

             {/* Complexity Badge */}
             {complexity && <ComplexityBadge score={complexity} />}

             {/* --- NEW: Plagiarism Score Display --- */}
             {plagiarism_score !== undefined && (
               <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${
                   plagiarism_score > 50 ? 'bg-red-50 border-red-200 text-red-800' : 
                   plagiarism_score > 20 ? 'bg-yellow-50 border-yellow-200 text-yellow-800' : 
                   'bg-green-50 border-green-200 text-green-800'
               }`}>
                 <AlertTriangle size={20} />
                 <div>
                   <div className="text-xs font-bold opacity-70 uppercase tracking-wide">Plagiarism Check</div>
                   <div className="font-bold text-lg">
                     {plagiarism_score}% Match 
                     <span className="text-xs font-normal opacity-80 ml-2">
                       {plagiarism_score > 50 ? "(High Risk)" : plagiarism_score > 20 ? "(Review Needed)" : "(Clean)"}
                     </span>
                   </div>
                 </div>
               </div>
          ...
          <div>
            <div className="rounded-lg overflow-hidden h-full flex flex-col border border-slate-200 shadow-sm">
              {/* Header using 'Biscay' Color */}
              <div className="bg-[#1B3147] p-3 text-xs text-white flex justify-between items-center">
                <span className="font-bold flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-[#40E0D0]"></span>
                  Generated Code ({method_used})
                </span>
                <span className="uppercase opacity-80 px-2 py-0.5 bg-white/10 rounded font-mono">{language}</span>
              </div>
              ...
              <div className="flex-1 bg-[#1e1e1e] overflow-hidden">
                <SyntaxHighlighter 
                  language={language} 
                  style={vscDarkPlus} 
                  customStyle={{ margin: 0, height: '100%', fontSize: '0.85rem', padding: '1rem' }} 
